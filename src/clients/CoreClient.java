...
public class CoreClient {
	...
	protected static int getRegionServer(String ipAddress) {
	...
	protected static String getIpAddressInput() {
		...
		while (CoreClient.getRegionServer(ipAddress) < 0) {
			if(CoreClient.getRegionServer(ipAddress) == -1) {
	...
	protected static String getServerName(int registryPort) {
		switch(registryPort) {
			case 1098:
				return "/NA";
			case 1099:
	...
	protected static synchronized void playerLog(String logStatement, String uName, String ipAddress) {
		 ...
		 String serverRegion = getServerName(registryPort);
		 try{
	...
	protected static synchronized void adminLog(String logStatement, String uName, String serverToConnect) {
		 ArrayList<String> REGION_LIST = new ArrayList<String>(Arrays.asList("/NA","/EU","/AS"));
		 if(!REGION_LIST.contains(serverToConnect)) serverToConnect = "/UNRESOLVED";
		 DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
		 LocalDateTime tStamp = LocalDateTime.now(); 
